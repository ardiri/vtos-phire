...
HelpInitialize()
{
  ...
  // draw the help
  {
    ...
    GfxSetFont(gfx_palmosNormalFont);
    {
      ...
      // general text
      x = 4;
      strcpy(str, "The building is on fire!");
      ptrStr = str;
      ...
      x = 4;
      strcpy(str,
"Guide your survivors to the ambuland using your trampoline for immediate \
treatment.");
      ptrStr = str;
      while (strlen(ptrStr) != 0) {
        BYTE count = GfxGetWordWrap(ptrStr, region.extent.x-x);
      ...
      x = 4;
      strcpy(str,
"Using the stylus or keys, move your trampoline left and right and guide the \
survivors to move in the path shown in the diagram above.");
      ptrStr = str;
      while (strlen(ptrStr) != 0) {
        BYTE count = GfxGetWordWrap(ptrStr, region.extent.x-x);
