    ...
    case EVT_INLAY_SELECT:
         ...
         switch (event->para1) 
         {
           case INLAY_EXIT:
                ...
                switch (globals->prefs->game.phire.gameType) 
                {
                  case GAME_A:
  ...
  switch (event->eventType)
  {
    case EVT_FORM_OPEN:
         ...
         // display the high score/code to the user
         {
           ...
           GfxDrawString(str, strlen(str), x, y, gfxPaint);
           y += GfxGetFontHeight();
           strcpy(str, (globals->prefs->game.phire.gameType == GAME_A) 
                       ? "GAME A" : "GAME B");
           GfxDrawString(str, strlen(str), x, y, gfxPaint);
           y += GfxGetFontHeight();
           if (globals->prefs->game.phire.gameType == GAME_B) 
             globals->prefs->game.gameScore += 1000;
           {
             BYTE  code[16] = "JOQWHBVXUWMNGKLI";
